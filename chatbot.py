...
class CustomerSupportBot:
    ...
    def process_knowledge_base(self, text, embedding_model_name="all-MiniLM-L6-v2"):
        """Process and vectorize the knowledge base using SentenceTransformerEmbeddings"""
        try:
...
- Summarize your answer in a concise paragraph.
- Do not use bullet points or lists; write in full sentences.
...
Please provide a helpful, professional response and summary focused on Paystack services. If you cannot answer based on the context provided, suggest contacting Paystack support at support@paystack.com or +234 1 700 5000."""
    ...
    def generate_response_gemini(self, query, context, api_key):
        """Generate response using Gemini-1.5-flash with max_tokens support"""
        try:
...
def setup_sidebar():
    ...
    if st.session_state.knowledge_base_loaded and "GPT" in model_option and st.session_state.vectorstore is None:
        with st.spinner("Processing knowledge base for RAG..."):
            bot = CustomerSupportBot()
            vectorstore = bot.process_knowledge_base(st.session_state.knowledge_content)
            if vectorstore:
